...
import { MessageCircle, PlusCircle, Trash2, TrendingUp } from "lucide-react";
...
export default function ChatbotAdminPanel() {
  ...
  const chartData = useMemo(
    () => [
      { key: "user", label: "Pertanyaan User", value: stats.totalUserMessages },
      { key: "bot", label: "Jawaban Bot", value: stats.totalBotMessages },
      { key: "faq", label: "Jawaban FAQ", value: stats.totalFaqMatched },
    ],
    [stats]
  );
  ...
  return (
    <section className="bg-white border border-border rounded-2xl shadow-soft p-5 space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted mt-1 max-w-xl">
              Pantau performa chatbot dan atur pertanyaan–jawaban yang akan dijawab otomatis oleh MasBot.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,2.1fr)_minmax(0,1.5fr)] items-stretch">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div className="border border-border rounded-xl px-3 py-2 bg-slate-50/60">
            ...
            <p className="text-lg font-semibold text-brand">{stats.totalSessions}</p>
          </div>
          <div className="border border-border rounded-xl px-3 py-2 bg-slate-50/60">
            <p className="text-muted mb-1">Pertanyaan User</p>
            <p className="text-lg font-semibold text-brand">{stats.totalUserMessages}</p>
          </div>
          <div className="border border-border rounded-xl px-3 py-2 bg-slate-50/60">
            <p className="text-muted mb-1">Jawaban Bot</p>
            <p className="text-lg font-semibold text-brand">{stats.totalBotMessages}</p>
          </div>
          <div className="border border-border rounded-xl px-3 py-2 bg-slate-50/60">
            <p className="text-muted mb-1">Jawaban dari FAQ</p>
            <p className="text-lg font-semibold text-brand">{stats.totalFaqMatched}</p>
          </div>
        </div>
        ...
        <div className="border border-border rounded-xl px-4 py-3 bg-slate-50/80 flex flex-col">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-xs font-semibold text-brand">Diagram Aktivitas Chatbot</p>
            </div>
            <span className="text-[10px] text-muted">Ringkasan interaksi MasBot</span>
          </div>
          ...
          <div className="w-full h-40">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 10 }}>
                ...
                />
                <Area type="monotone" dataKey="value" stroke="none" fill="url(#chatbotArea)" />
                <Line type="monotone" dataKey="value" stroke="#4C74B9" strokeWidth={2} dot={{ r: 3 }} activeDot={{ r: 4 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>

          <p className="mt-3 text-[10px] text-muted">
            Grafik memperlihatkan perbandingan jumlah pertanyaan user, jawaban bot, dan jawaban yang diambil dari FAQ.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-[minmax(0,1.4fr)_minmax(0,1.6fr)] items-start pt-4 border-t border-border/60 mt-4">
        <div>
          <h3 className="text-sm font-semibold text-brand mb-2">Tambah Pertanyaan &amp; Jawaban Baru</h3>
        ...
        <div>
          <h3 className="text-sm font-semibold text-brand mb-2">Pertanyaan yang Paling Sering Dipakai</h3>

          {topFaqs.length === 0 ? (
            <p className="text-xs text-muted">Belum ada data FAQ. Tambahkan pertanyaan terlebih dahulu.</p>
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] text-muted mt-1">
                      Dipakai <span className="font-semibold">{f.timesUsed}x</span>
                    </p>
                  </div>
                  <button
                    type="button"
